...
/*
    Definição de tipos e protótipos de funções de callback necessárias para
    manipulação dos itens da seção.
*/
...
/**
 * @brief Função que adiciona um item à seção.
 *        É responsável por toda manipulação de memória para que isso aconteça
 *        de forma correta.
 *
...
/**
 * @brief Função que imprime item/itens da seção de acordo com uma chave de
 *        comparação.
 *        Dentro dessa função, é feita a comparação entre a chave e o item de
 *        acordo com a função de callback comparação.
 *        Para imprimir o item, é utilizada a função de callback imprime.
 *
 * @param sec Ponteiro para a seção.
 * @param chave Ponteiro para a chave de comparação. Observe que é um ponteiro
 *              void, ou seja, pode ser qualquer tipo de dado.
 * @param compara Função de callback para comparar um item com a chave
 *                informada.
